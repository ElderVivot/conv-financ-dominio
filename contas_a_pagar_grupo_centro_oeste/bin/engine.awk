BEGIN { 
	...
	print "Documento;CNPJ Fornecedor;Emissao;Vencimento;Banco Oco. Extrato;Data Pagto;Data Oco. Extrato;Valor Pago;Valor Desconto;Valor Juros;Nome Fornecedor;Titulo;Outros;Tipo Pagto;Codigo Empresa Dominio" >> "temp\\pagtos_agrupados.csv"
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			if( index(empresa, "OESTE") ){
				codi_emp = ""
				codi_emp = 1511
			}
			if( index(empresa, "SOLDA") ){
				codi_emp = ""
				codi_emp = 1512
			}
			...
			outros = ""
			outros = Trim($pos_natureza_pagto)
			outros = subsCharEspecial(outros)
			outros = upperCase(outros)
			...
			if( baixa != "NULO" && int(valor_pago) > 0 ){
				print nota, "'" cnpj_forn_cli, emissao, vencimento, banco, baixa, baixa_extrato, valor_pago, valor_desconto, valor_juros,
      				  forn_cli, nota_completo_orig, outros, tipo_pagto, codi_emp >> "temp\\pagtos_agrupados.csv"
			}
