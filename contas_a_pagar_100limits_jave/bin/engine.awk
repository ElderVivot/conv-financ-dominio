BEGIN { 
	...
	print "Documento;CNPJ Fornecedor;Emissao;Banco Oco. Extrato;Data Pagto;Data Oco. Extrato;Valor Pago;Valor Desconto;Valor Juros;Nome Fornecedor;Titulo;Codigo Empresa Dominio" >> "temp\\pagtos_agrupados.csv"
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			if( substr(empresa, 1, 8) == "100 LIMI" ){
				codi_emp = ""
				codi_emp = substr(empresa, length(empresa) - 5)
				if(codi_emp == "FILIAL")
					codi_emp = 2
				if(codi_emp == "MATRIZ")
					codi_emp = 3
			...
			if( baixa != "NULO" && int(valor_pago) > 0 ){
				print nota, "'" cnpj_forn_cli, emissao, banco, baixa, baixa_extrato, valor_pago, valor_desconto, valor_juros,
      				  forn_cli, nota_completo_orig, codi_emp >> "temp\\pagtos_agrupados.csv"
			}
