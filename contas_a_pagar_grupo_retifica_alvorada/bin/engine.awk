BEGIN { 
	...
	system("dir /b entrada\\*.txt > bin\\listacsv.txt")
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			forn_cli = ""
			forn_cli = Trim(substr($0,32,60))
			forn_cli = subsCharEspecial(forn_cli)
			forn_cli = upperCase(forn_cli)
			
			nota_completo = ""
			nota_completo = Trim(substr($0,132,9))
			...
			baixa_temp = ""
			baixa_temp = Trim(substr($0,1,10))
			baixa_temp = FormatDate(baixa_temp)
			baixa_temp = isDate(baixa_temp)
			
			emissao = ""
			emissao = Trim(substr($0,13,10))
			emissao = FormatDate(emissao)
			emissao = isDate(emissao)
			emissao = IfElse( emissao == "NULO", "", emissao )
			
			vencimento = ""
			vencimento = Trim(substr($0,121,10))
			vencimento = FormatDate(vencimento)
			vencimento = isDate(vencimento)
			vencimento = IfElse( vencimento == "NULO", "", vencimento )
			
			if( ( emissao != "NULO" || vencimento != "NULO" ) && baixa_temp != "NULO" ){
				baixa = ""
				baixa = Trim(substr($0,1,10))
			...
			valor_original = ""
			valor_original = Trim(substr($0,94,13))
			...
			valor_pago = ""
			valor_pago = Trim(substr($0,107,13))
			...
			valor_juros = ""
			valor_juros = Trim(substr($0,142,10))
			valor_juros = FormataCampo("double", valor_juros, 12, 2)
			
			valor_desconto = ""
			valor_desconto = Trim(substr($0,153,10))
			...
			if( index(campo_1, "AGENCIA") > 0){
				banco_arquivo = ""
				banco_arquivo = Trim($1)
				...
				}else if( index(banco_arquivo, "TRI") > 0 && index(banco_arquivo, "BANCO") > 0 ){
					num_banco_arq = 634
			...
			if( index(campo_1, "CONTA") > 0){
				conta_corrente_arq = ""
				conta_corrente_arq = Trim($1)
				conta_corrente_arq = subsCharEspecial(conta_corrente_arq)
				conta_corrente_arq = upperCase(conta_corrente_arq)
				conta_corrente_arq = split(conta_corrente_arq, conta_corrente_arq_v, "-")
				
				conta_corrente_arq = conta_corrente_arq_v[2]
				banco_arquivo = banco_arquivo "-" conta_corrente_arq
			}
			...
			# PAGOS
			if( ( emissao != "" || vencimento != "" ) && int(valor_pago) > 0 && _comp_ini <= baixa_temp && baixa_temp <= _comp_fim ){
			...
			# RECEBIMENTOS
			if( ( emissao != "" || vencimento != "" ) && int(valor_recebido) > 0 && _comp_ini <= baixa_temp && baixa_temp <= _comp_fim ){
