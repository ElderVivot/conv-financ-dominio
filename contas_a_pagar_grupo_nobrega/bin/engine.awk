BEGIN { 
		...
		while ((getline < fileofx) > 0) {
			...
			# ESTRUTURA SEM TAG FINAL
				...
				if( substr(ofx, 1, 8) == "<trnamt>" ){
					...
					if( index(valor_transacao, ",") == 0 )
						valor_transacao = substr(valor_transacao, 1, length(valor_transacao)-2) "," substr(valor_transacao, length(valor_transacao)-1)
				}
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			}else if( index(banco_arquivo, "ITAU") > 0 && index(banco_arquivo, "COMPUSAT") == 0 ){
				num_banco_arq = 341
				banco_arquivo = "ITAU"
			}else if( index(banco_arquivo, "ITAU") > 0 && index(banco_arquivo, "COMPUSAT") > 0 ){
				num_banco_arq = 341
			...
			}else if( index(banco_arquivo, "CARTEIRA") > 0 ){
				num_banco_arq = 999
				banco_arquivo = "DINHEIRO"
			}else{
				num_banco_arq = 0
