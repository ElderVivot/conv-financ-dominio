BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		# PRIMEIRO WHILE VE ONDE ESTÁ O CAMPO DO CENTRO DE CUSTO
		while ((getline < filecsv) > 0) {
			
			num_linha++

			CentroCusto[num_linha] = $1
			
		} close(filecsv)
		
		# SEGUNDO WHILE VAI IMPRIMIR O ARQUIVO
		while ((getline < filecsv) > 0) {
			...
			if ( Trim(toupper($1)) == toupper("DATA") ){
				...
				categoria = ""
				categoria = CentroCusto[num_linha2-1]

				load_columns();
				continue;
			}
			
			texto_for = "TIPO DE CONTA"
			texto_cnpj_for = "---------"
			texto_nota = "---------"
			texto_emissao = "---------"
			texto_venc = "---------"
			...
			texto_categoria = "---------"
			texto_banco_arquivo = "OBSERVACAO/JUROS"
			texto_empresa = ""
			texto_tipo_pagto = "---------"
