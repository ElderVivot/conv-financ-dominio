BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < filecsv) > 0) {
			...
			pos_nota = $IfElse( int(NumColuna(texto_nota)) > 0, int(NumColuna(texto_nota)), 15 )
			...
			pos_baixa = $IfElse( int(NumColuna(texto_baixa)) > 0, int(NumColuna(texto_baixa)), 14 )
			pos_valor_original = $IfElse( int(NumColuna(texto_valor_original)) > 0, int(NumColuna(texto_valor_original)), 999 )
			pos_valor_pago = $IfElse( int(NumColuna(texto_valor_pago)) > 0, int(NumColuna(texto_valor_pago)), 16 )
			...
			pos_obs = $IfElse( int(NumColuna(texto_obs)) > 0, int(NumColuna(texto_obs)), 8 )
			...
			campo_5 = ""
			campo_5 = Trim($5)
			campo_5 = subsCharEspecial(campo_5)
			campo_5 = upperCase(campo_5)

			if(index(campo_5, "BANCO") > 0){
				banco_arquivo = ""
				banco_arquivo = Trim($6)
