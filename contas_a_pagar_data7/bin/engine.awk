BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			banco_arquivo = ""
			banco_arquivo = Trim( $8 )
			banco_arquivo = subsCharEspecial(banco_arquivo)
			banco_arquivo = upperCase(banco_arquivo)
			
			BancoArquivo[file, linha] = banco_arquivo
			
		} close(file)
	} close(ArquivosCsv)
	
	print "Nota;CNPJ Fornecedor;Data Vencimento;Banco Arquivo;Banco Oco. Extrato;Data Pagto;Data Oco. Extrato;Valor Pago;Valor Desconto;Valor Juros;Nome Fornecedor;Obs;Cod. Emp. Dom." >> "temp\\pagtos_agrupados.csv"
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			empresa = ""
			empresa = Trim($pos_emp)
			empresa = subsCharEspecial(empresa)
			empresa = upperCase(empresa)
			
			if( empresa == "BR BLEND 1"){
				codi_emp = ""
				codi_emp = 81
			}
			
			if( empresa == "BR BLEND 2"){
				codi_emp = ""
				codi_emp = 69
			}
			...
			if( valor_juros == "0,00" && valor_original_int < valor_pago_int ){
			...
			if( valor_desconto == "0,00" && valor_original_int > valor_pago_int ){
			...
			banco_arq = BancoArquivo[file, linha_2+1]
			if(index(banco_arq, "ITAU") > 0)
				banco_arq = "ITAU"
			if(index(banco_arq, "CAIXA") > 0 && index(banco_arq, "CXA") > 0)
				banco_arq = "DINHEIRO"
			...
			if( baixa != "NULO" && int(valor_pago) > 0 ){
				print nota, "'" cnpj_forn_cli, venc, banco_arq, banco, baixa, baixa_extrato, valor_pago, valor_desconto, valor_juros,
      				  forn_cli, obs, codi_emp >> "temp\\pagtos_agrupados.csv"
			}
