...
# ------------------ FUN��O MATCH(express�o, combina��o ser encontrada) --> EXPLICA��O -----------------------
#O match() � bastante parecida com o index(), exceto que em vez de procurar por uma substring como o index() faz, ele procura por uma express�o regular. 
#A fun��o match() ir� retornar a posi��o inicial da combina��o, ou zero se n�o houver combina��o. Al�m disso, o match() ir� configurar duas vari�veis 
#chamadas RSTART e RLENGTH. RSTART cont�m o valor de retorno (a localiza��o da primeira combina��o), e RLENGTH ir� conter seu comprimento em caracteres 
#(ou -1 se nenhuma combina��o for encontrada). Veja um exemplo da chamada ao match():
#print match("Elder ama Amanda",/ama/), RSTART, RLENGTH
#O awk ir� escrever: 7 7 3
#O primeiro 7 � a posi��o inicial da combina��o
#O segundo 7 � o local onde encontrou a primeira combina��o
#O 3 � o tamanho do "ama"

# ----------------- FUN��O INDEX(express�o, caracter a ser encontrado) --> EXPLICA��O -------------------------
#index("Amor", "r"): procura na string "Amor" a primeira ocorr�ncia da string "r" e retorna a posi��o onde foi encontrada dentro da string "Amor" que 
#neste caso seria a quarta

# ----------------- FUN��O SPLIT (express�o, vetor, separador) --> EXPLICA��O ------------------------------
#A fun��o split() divide a string em partes separadas por campos de acordo o separador, e armazena as partes no array, que no caso � o vetor. 
#O valor do separador � uma express�o regular que descreve onde dividir a string. Se o valor do separador � omitido, o valor de FS � usado.
# Se o seu valor for a string nula (""), ent�o cada caracter torna-se um campo separador. O default � " ". 
...
# SELECIONA COLUNAS QUANDO É SEPARADO SOMENTE POR ESPAÇO
...
function FormatDate(inDate, charSeparador, tpData) {
	...
	if (match(inDate, charSeparador) > 0) {
		...
		if(tpData == "US"){
			...
			#Se o dia for maior que dois, ou seja, se tiver hor�rio tamb�m
			if( length(rtD) > 2 )
				rtD = substr(rtD, 1, 2)
			else
				rtD = rtD
			...
			#Se o ano for maior que quatro, ou seja, se tiver hor�rio tamb�m
			if( length(rtY) > 4 )
				rtY = substr(rtY, 1, 4)
			else
				rtY = rtY
...
function transformaDataEmDias(argData){
	...
	for(x = 0; x < mes_data; x++) { # calcula dias dos MESES ANTERIORES
		...
		if ( (x == 1) || (x == 3) || (x == 5) || (x == 7) || (x == 8) || (x == 10) || (x == 12) ){  # verifica se � 31
			qtdDias += 31
			continue
		}
		else if (x == 2) {
