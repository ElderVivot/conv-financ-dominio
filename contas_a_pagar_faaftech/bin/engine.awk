BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		#while ((getline < file) > 0) {
			...
			if ( Trim(toupper($1)) == toupper("Nro Compensacao/Acerto") ){
				load_columns();
				continue;
			}
			
			
			texto_for = "Nome Parceiro"
			texto_cnpj_for = "CNPJ / CPF (Financeiro)"
			texto_nota = "Nro Nota"
			texto_emissao = "Dt. Entrada e Saida"
			texto_venc = "Dt. Vencimento"
			texto_baixa = "Data Baixa"
			texto_valor_original = "---------"
			texto_valor_pago = "Vlr Baixa"
			texto_valor_recebido = "---------"
			texto_valor_desc = "Vlr Desconto"
			texto_valor_juros = "Vlr Juros"
			texto_valor_multa = "Vlr Multa"
			texto_obs = "Descricao (Tipo de Operacao)"
			texto_categoria = "Descricao (Natureza)"
			...
			if( banco_arquivo == "CAIXA INTERNO" && banco == "NAO ENCONTROU NO OFX" )
				banco = "DINHEIRO"
