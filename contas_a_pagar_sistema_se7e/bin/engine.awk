BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < filecsv) > 0) {
			...
			if ( Trim(toupper($1)) == toupper("Pagamento") ){
				load_columns();
				continue;
			}
			...
			texto_valor_original = "Vlr.Titulo"
			texto_valor_pago = "Vlr.Pago"
			...
			pos_nota = $IfElse( int(NumColuna(texto_nota)) > 0, int(NumColuna(texto_nota)), 5 )
			...
			campo_1 = split(campo_1, campo_1_v, "-")

			codigo_fornecedor = campo_1_v[1]
			codigo_fornecedor = int(soNumeros(codigo_fornecedor))
			if(codigo_fornecedor > 0 && campo_1 > 1 ){ # o maior que 1 é pra identificar se tem o "-" no campo ou não, quando o valor é 1 quer dizer que não tem
				forn_cli = ""
				forn_cli = Trim(campo_1_v[2] " " campo_1_v[3] " " campo_1_v[4] " " campo_1_v[5] " " campo_1_v[6])
