BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
			...
			texto_valor_pago = "Valor do pagamento (R$)"
	...
	while ( (getline < "temp\\extrato_cartao.csv") > 0 ) {
		...
		if( ( operacao_3 == "-" || operacao_3 == "Operacao") && pagou_no_banco != 1 && ( index(historico_2, "SISPAG") == 0 || index(historico_2, "SALAR") > 0 ) )
		#if( pagou_no_banco != 1 )
