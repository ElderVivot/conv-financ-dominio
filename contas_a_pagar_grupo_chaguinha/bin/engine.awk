BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
			...
			pos_for = $IfElse( int(NumColuna(texto_for)) > 0, int(NumColuna(texto_for)), 3 )
			...
			pos_baixa = $IfElse( int(NumColuna(texto_baixa)) > 0, int(NumColuna(texto_baixa)), 6 )
