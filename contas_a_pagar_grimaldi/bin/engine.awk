BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		if( forma_pagto == "DINHEIRO" ){
			BancoArquivo[ nro_lote ] = forma_pagto
		} else if( forma_pagto == "PAGAMENTO" ){
			
			nome_banco = ""
			nome_banco = Trim( $3 )
			if( index(nome_banco, "CAIXA") > 0 ){
				nome_banco = "CAIXA"
			}
			...
			BancoArquivo[ nro_lote ] = nome_banco "|" agencia "|" conta
		} else
			continue
	}close(ArquivosCsv)
	...
	while ((getline < ArquivosCsv) > 0) {
			...
			texto_valor_pago = "Pago"
			texto_valor_recebido = "---------"
			texto_valor_desc = "Desc."
			...
			pos_emissao = $IfElse( int(NumColuna(texto_emissao)) > 0, int(NumColuna(texto_emissao)), 5 )
			...
			pos_valor_original = $IfElse( int(NumColuna(texto_valor_original)) > 0, int(NumColuna(texto_valor_original)), 11 )
			pos_valor_pago = $IfElse( int(NumColuna(texto_valor_pago)) > 0, int(NumColuna(texto_valor_pago)), 18 )
			pos_valor_recebido = $IfElse( int(NumColuna(texto_valor_recebido)) > 0, int(NumColuna(texto_valor_recebido)), 999 )
			pos_valor_desc = $IfElse( int(NumColuna(texto_valor_desc)) > 0, int(NumColuna(texto_valor_desc)), 16 )
			pos_valor_juros = $IfElse( int(NumColuna(texto_valor_juros)) > 0, int(NumColuna(texto_valor_juros)), 13 )
			pos_valor_multa = $IfElse( int(NumColuna(texto_valor_multa)) > 0, int(NumColuna(texto_valor_multa)), 14 )
			...
			banco_arquivo = ""
			banco_arquivo = BancoArquivo[nro_lote_2]
