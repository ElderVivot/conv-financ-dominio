BEGIN { 
	...
	pos_tipo_pagto = 999
	pos_valor_adiantamento = 27
	
	print "Documento;CNPJ Fornecedor;Emissao;Vencimento;Banco Oco. Extrato;Data Pagto;Data Oco. Extrato;Valor Pago;Valor Desconto;Valor Juros;Nome Fornecedor;Titulo;Categoria" >> "temp\\pagtos_agrupados.csv"
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			campo_2 = ""
			campo_2 = Trim($2)
			campo_2 = subsCharEspecial(campo_2)
			campo_2 = upperCase(campo_2)
			
			if( substr(campo_2, 1, 12) == "DESCRICAO CO" ){
				categoria = ""
				categoria = Trim($8)
			...
			emissao = IfElse(emissao == "NULO", "01/01/1900", emissao)
			...
			valor_adiantamento = ""
			valor_adiantamento = Trim( $pos_valor_adiantamento )
			valor_adiantamento = FormataCampo("double", valor_adiantamento, 12, 2)
			valor_adiantamento_int = int(soNumeros(valor_adiantamento))
			...
			if( valor_adiantamento_int > 0 ){
				valor_pago = valor_adiantamento
				baixa = vencimento
			}
			...
			if( baixa != "NULO" && int(valor_pago) > 0 ){
				print nota, "'" cnpj_forn_cli, emissao, vencimento, banco, baixa, baixa_extrato, valor_pago, valor_desconto, valor_juros,
      				  forn_cli, nota_completo_orig, categoria >> "temp\\pagtos_agrupados.csv"
			}
