BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
			...
			texto_nota = "---------"
			texto_emissao = "---------"
			texto_venc = "Data Venc."
			texto_baixa = "Data Baixa"
			texto_valor_original = "---------"
			texto_valor_pago = "Valor Pag."
			...
			texto_banco_arquivo = "---------"
			texto_empresa = ""
			texto_tipo_pagto = "Forma"
			texto_natureza_pagto = "-------------"
			texto_tipo_rec_ou_pag = "Tipo"
			...
			pos_baixa = $IfElse( int(NumColuna(texto_baixa)) > 0, int(NumColuna(texto_baixa)), 10 )
			...
			pos_tipo_pagto = $IfElse( int(NumColuna(texto_tipo_pagto)) > 0, int(NumColuna(texto_tipo_pagto)), 3 )
			...
			pos_tipo_rec_ou_pag = $IfElse( int(NumColuna(texto_tipo_rec_ou_pag)) > 0, int(NumColuna(texto_tipo_rec_ou_pag)), 2 )
			...
			valor_recebido = ""
			valor_recebido_int = 0
			if( pago_recebido == "RECEBER" ){
				valor_recebido = valor_pago
				valor_recebido_int = valor_pago_int
				valor_pago = "0,00"
				valor_pago_int = 0
			}
