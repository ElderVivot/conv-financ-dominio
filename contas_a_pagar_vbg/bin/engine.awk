BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < filecsv) > 0) {
			...
			pos_emissao = $IfElse( int(NumColuna(texto_emissao)) > 0, int(NumColuna(texto_emissao)), 11 )
			...
			pos_valor_pago = $IfElse( int(NumColuna(texto_valor_pago)) > 0, int(NumColuna(texto_valor_pago)), 15 )
			...
			vencimento_temp = ""
			vencimento_temp = Trim($1)
			vencimento_temp = FormatDate(vencimento_temp)
			vencimento_temp = isDate(vencimento_temp)
			if(vencimento_temp != "NULO"){
				vencimento = ""
				vencimento = vencimento_temp
			}
