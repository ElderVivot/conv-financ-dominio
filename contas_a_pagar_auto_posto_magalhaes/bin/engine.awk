BEGIN { 
	...
	while ((getline < ArquivosCsv) > 0) {
		...
		while ((getline < file) > 0) {
			...
			forn_cli_temp = ""
			forn_cli_temp = Trim(substr($0,45,25))
			forn_cli_temp = subsCharEspecial(forn_cli_temp)
			forn_cli_temp = upperCase(forn_cli_temp)

			forn_cli_temp = split(forn_cli_temp, forn_cli_v, "-")

			forn_cli = Trim(forn_cli_v[2] " " forn_cli_v[3] " " forn_cli_v[4] " " forn_cli_v[5])
			
			nota_completo = ""
			nota_completo = Trim(substr($0,8,9))
			...
			valor_original = ""
			valor_original = Trim(substr($0,71,9))
			...
			valor_desconto = ""
			valor_desconto = Trim(pos_valor_desconto)
			...
			banco_arquivo = ""
			banco_arquivo = Trim(substr($0,162,16))
			...
			# PAGOS
			if( int(valor_pago_int) > 0 && _comp_ini <= baixa_temp && baixa_temp <= _comp_fim ){
